...
static const char *pcmvis_get_name(void *) { return "PCM Visualizer (Skeleton)"; }
...
{
    auto *s = static_cast<pcmvis*>(data);
    if (s->vbuf) gs_vertexbuffer_destroy(s->vbuf);
    delete s;
}

static uint32_t pcmvis_get_width(void *)  { return 800; }
static uint32_t pcmvis_get_height(void *) { return 200; }

static void ensure_vbuf(pcmvis *s, size_t n)
{
    if (s->vbuf && gs_vertexbuffer_get_num_vertices(s->vbuf) == n) return;
    if (s->vbuf) gs_vertexbuffer_destroy(s->vbuf);
...
static void pcmvis_video_render(void *data, gs_effect_t *)
{
    ...
    vec4 col; vec4_from_rgba(&col, s->color);
